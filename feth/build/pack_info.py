    ...
    def __init__(self, *, data=None, index=None, path: pathlib.Path = None):
        ...
        if data:
            ...
            self.path = pathlib.Path(
                LAYERED_FS_DIR,
                MOD_DIR,
                path.split(b"\0", 1)[0].decode("utf-8")[len("rom:/") :],
            )
        else:
            self.index = index
            self.path = path
...
def build_distrib():
    ...
    if ATTACH_PATH.exists():
        for file in ATTACH_PATH.glob("*"):
            shutil.copy(file, MODS_PATH)
    ...
    for cur_path in mod_dir_path.glob("**/*"):
        if cur_path.is_file():
            if cur_path in path_index_map:
                ...
                if not index_entry_map[
                    cur_index
                ].entry_data:  # entry_data not populated means it's already mods entry
                    print(
                        f"WARNING: conflict in {MOD_DIR} - {cur_index} appeared more than once"
                    )
                ...
                if file_index_match:
                    ...
                    if cur_file_index in index_entry_map:
                        ...
                        if not index_entry_map[cur_file_index].entry_data:
                            print(
                                f"WARNING: conflict in {MOD_DIR} - {cur_file_index} appeared more than once"
                            )
                    else:
                        print(f"Add: {cur_file_index} - {cur_path}")
                    index_entry_map[cur_file_index] = InfoEntry(
                        index=cur_file_index, path=cur_path
                    )
                else:
    ...
    with open(
        os.path.join(LAYERED_FS_DIR, latest_patch_name, "INFO0.bin"), "wb"
    ) as out_info0_file:
        for info_entry in sorted(index_entry_map.items(), key=lambda item: item[0]):
            out_info0_file.write(info_entry[1].data())

    with open(
        os.path.join(LAYERED_FS_DIR, latest_patch_name, "INFO2.bin"), "wb"
    ) as out_info2_file:
        out_info2_file.write(
            struct.pack("<QQ", len(index_entry_map), info1_entry_count)
        )
